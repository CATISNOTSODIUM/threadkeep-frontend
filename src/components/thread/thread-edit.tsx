...
export default function ThreadEditModal(props: {threadProps: Thread, setIsToggle: React.Dispatch<React.SetStateAction<boolean>>}) {
    ...
    return (
        <div data-color-mode="light" className='fixed top-1/6 left-1/6 w-2/3 min-h-1/2 h-fit  z-50 p-10 bg-white rounded-xl shadow-xl'>
            ...
            <button className='bg-yellow-200 font-bold text-black py-2 w-1/4 min-w-24 mr-2 my-2 rounded-full' onClick={submitThread}>Submit</button>
            <button className='bg-yellow-100 font-bold text-black py-2 w-1/4 min-w-24 mr-2 my-2 rounded-full' onClick={() => setIsToggle(false)}>Cancel</button>
